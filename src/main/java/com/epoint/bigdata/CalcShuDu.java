...
public class CalcShuDu {
	public static void main(String[] args) {
		...
		for (CandiInfo info : equalCandi) {
			...
			System.out.println("开始进行试探：data=" + data[0] + ", " + data[1] + " 位置：" + aRow + "-" + aColumn + ", " + bRow
					+ "-" + bColumn);
	 ...
	 * @return
	 */
	private static boolean isRight(int[][][] candi, int[][] sudo, int aRow, int aColumn, int bRow, int bColumn,
			int data0, int data1) {
				...
				while (iterator.hasNext()) {
					...
					if (set.size() == 0) {
						...
						if (set.size() == 1) {
							...
							flag = true;
							modifySudoAndCandi(candi, sudo, Integer.parseInt(ks[0]), Integer.parseInt(ks[1]),
									set.stream().mapToInt(Integer::intValue).toArray()[0]);
						}
					}
				}
			}
				...
				while (iterator.hasNext()) {
					...
					if (set.size() == 0) {
						...
						if (set.size() == 1) {
							String[] ks = tKey.split("-");
							flag = true;
							modifySudoAndCandi(candi, sudo, Integer.parseInt(ks[1]), Integer.parseInt(ks[0]),
									set.stream().mapToInt(Integer::intValue).toArray()[0]);
						}
					}
				}
	...
	/**
	 * 校验这个数独是不是还满足数独的特点 思路： 1. 校验行和列有没有重复的数字 2.
	 * 校验数独是0的格子，对应的隐形数组还有没有值，如果没有候选值，肯定是某一个地方填错了
	 * 
	 * @param candi
	 *            隐形数组
	 * @param sudo
